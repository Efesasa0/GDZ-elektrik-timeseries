...
def pre_imputation(all_data):
    
    # TARGET TRAIN should be interpolate
    all_data['target'] = all_data['target'].interpolate().round()
    
    # BILDIRIMLI_SUM TRAIN should interpolate
    if 'bildirimli_sum' in all_data.columns:
        all_data['bildirimli_sum'] = all_data['bildirimli_sum'].interpolate().round()
...
def create_holiday_features(all_data, holiday_dict=None, holiday_in_n_days_ls=None):   
    ...
    if holiday_in_n_days_ls is None:
        holiday_in_n_days_ls = [1,2,4,8,16]
    ...
    all_data['is_holiday'] = ''
    for cat in all_data['holiday'].cat.categories:
        if cat != 'no_holiday':
            all_data.loc[all_data['holiday'] == cat, 'is_holiday'] = 'yes_holiday'
        else:
            all_data.loc[all_data['holiday'] == cat, 'is_holiday'] = 'no_holiday'
    all_data['is_holiday'] = all_data['is_holiday'].astype('category')
    all_data = all_data.drop(columns=["holiday"])

    return all_data

def create_static_features(all_data, cols, categories, agg_types):
    for col in cols:
        for agg_type in agg_types:
            all_data[f"static_{col}_{agg_type}"] = all_data.groupby("item_id")[col].transform(agg_type)
        for cat in categories:
            all_data[f"static_{col}_{cat}_{agg_type}"] = all_data.groupby(["item_id",cat])[col].transform(agg_type)
    return all_data
...
def create_target_lag_features(all_data, lags):
    return create_lag_features(all_data, ['target'], lags)
                

def create_forward_lag_features(all_data, columns, lags):
    item_ls = list(all_data["item_id"].unique())
    ...
    for item_id in item_ls:
        ...
        for col in columns:
            temp = all_data[temp_index][col]
            for lag in lags:
                temp_lag = temp.shift(-lag).ffill()
                all_data.loc[temp_index, f'flag_{lag}_{col}'] = temp_lag
...
def validation_prep(df):
    ...
    # target_roll_n_ -->  0
    pattern = re.compile(r'^target_roll_([0-9]+)_[a-z]+$')
    cols = []
    for col in df.columns:
        match = pattern.match(col)
        if match:
    ...
    for item_id in item_ls:
        ...
        for col in cols:
            temp = df.loc[index, col].to_numpy()
            # we can keep the first value
            temp[1:] = 0.0
            df.loc[index, col] = temp
            print('cleaning',col, item_id)
...
def split_in_two(all_data, val_start_date, horizon):
    val_range = pd.date_range(val_start_date, periods=horizon, freq="D")
...
def evaluate_validation(model_dict, val, X_val, plot_validation, file=None, max_horizon=29, cat_features=None):
    for model_name in model_dict.keys():
        val[f'target_pred_{model_name}'] =  make_forecast(X_val, 
...
def train_predict(all_data, X_train, y_train, X_val, y_val, val, config_params,
    ...
    for model_name, params in models_params.items():
        ...
        if model_name=="xgboost":
            ...
            model_dict[model_name]['model'].fit(X_train, y_train, eval_set=[(X_train, y_train)], **params['fit'])
        ...
        if model_name=="lightgbm":
            ...
            eval_set=[(X_train, y_train)],**params['fit'])
            val['target_pred_lightgbm'] = make_forecast(X_val, model_name, model_dict, max_horizon=max_horizon) 
            
    start_of_val = val['timestamp'].min()

    impt_plot_file = f'{config_params["experiment_folder"]}/{config_params["experiment_hash"]}_importance_{start_of_val}.png'

    if print_feature_importance:
        plot_feature_importance(models_params, model_dict, list(X_train.columns), file=impt_plot_file)
    ...
    val.to_excel(f'{config_params["experiment_folder"]}/{config_params["experiment_hash"]}_val_df.xlsx')
    X_val.to_excel(f'{config_params["experiment_folder"]}/{config_params["experiment_hash"]}_X_val_df.xlsx')
    model_dict = evaluate_validation(model_dict, 
                                     val, 
                                     X_val, 
                                     plot_validation, 
                                     file=val_plot_file,
...
def smooth_target(df, col, window=7, method='moving_average', upsample_multiplier=1):
    item_ls = pd.unique(df['item_id'])
    for item_id in item_ls:
        item_index = df['item_id'] == item_id
        input_signal = df.loc[item_index, col]
        input_signal_upsampled = resample(input_signal, upsample_multiplier * len(input_signal))
        if method == 'moving_average':
            smoothed_signal = input_signal_upsampled.rolling(window=window, min_periods=1).mean()
        elif method == 'savgol':
            smoothed_signal = savgol_filter(input_signal_upsampled, window, 3)

        df.loc[item_index, col] = resample(smoothed_signal, len(input_signal))
        #elif method == 'exponential':
        #    return df[target_col].ewm(span=window, adjust=False).mean()
        #elif method == 'gaussian':
        #    from scipy.ndimage import gaussian_filter1d
        #    return pd.Series(gaussian_filter1d(df[target_col], sigma=window / 3))
        #else:
        #    raise ValueError("Invalid method. Choose 'moving_average', 'exponential', or 'gaussian'.")
    return df
...
def cook_yengecburger(config_params):
    ...
    # DATA PREPERATION
    all_data_org = union_train_test(train, test)

    # DATA IMPUTATION


    def create_features(all_data_org, cutoff_date):
        all_data = all_data_org.copy()

        if config_params["OUTLIER_METHOD"] is not None:
            outliers_in_train = (all_data['target'] > config_params["OUTLIER_LIMIT"]) &\
                               (all_data['timestamp'] < cutoff_date)
            if config_params["OUTLIER_METHOD"] == "CLIP":
                all_data.loc[outliers_in_train,'target'] = config_params["OUTLIER_LIMIT"]
            elif config_params["OUTLIER_METHOD"] == "REMOVE":
                all_data = all_data[all_data['target'] <= config_params["OUTLIER_LIMIT"]]
        ...
        target_values = all_data['target'].copy()
        # we remove target after cutoff to prevent contamination
        all_data.loc[all_data['timestamp'] >= cutoff_date, 'target'] = 0.0
        # FEATURE ENGINEERING
        ...
        if len(config_params["LAG_FEATURES"]) > 0:
            all_data = create_lag_features(all_data,
                                        config_params["LAG_FEATURES"],
                                        config_params["LAGS"])
        if len(config_params["TARGET_LAGS"]) > 0:
            #assert(min(config_params["TARGET_LAGS"]) > HORIZON)
            all_data = create_target_lag_features(all_data,
                                        config_params["TARGET_LAGS"])
             
        if len(config_params["FORWARD_LAG_FEATURES"]) > 0:
            all_data = create_forward_lag_features(all_data,
                                        config_params["FORWARD_LAG_FEATURES"],
                                        config_params["FORWARD_LAGS"])
        ...
        if len(config_params["STATIC_FEATURES"]) > 0:
            all_data = create_static_features(all_data, 
                                            config_params["STATIC_FEATURES"],
                                            config_params["STATIC_FEATURES_SUB_CATEGORIES"],
                                            config_params["STATIC_FEATURES_AGG_TYPES"])
        # put back
        all_data['target'] = target_values
        return all_data
    ...
    items = list(all_data_org["item_id"].unique())
    
    for fold, val_start_date in enumerate(config_params["VAL_START_DATES"]):

        all_data_in_fold = create_features(all_data_org, val_start_date)

        trn, val = split_in_two(all_data_in_fold, val_start_date, HORIZON)

        if config_params['SMOOTH_TARGET']:
            trn = smooth_target(trn, 'target', 
                                    window=config_params['SMOOTH_TARGET_WINDOW_SIZE'], 
                                    method=config_params['SMOOTH_TARGET_METHOD'],
                                    upsample_multiplier=config_params['SMOOTH_TARGET_UPSAMPLE_MULTIPLIER'])
        
        X_train, y_train, X_val, y_val = create_io_pairs(trn, val)
        trn_start, trn_end = trn["timestamp"].min().strftime('%Y-%m-%d'), trn["timestamp"].max().strftime('%Y-%m-%d')
        val_start, val_end = val["timestamp"].min().strftime('%Y-%m-%d'), val["timestamp"].max().strftime('%Y-%m-%d')
        #tst_start, tst_end = tst["timestamp"].min().strftime('%Y-%m-%d'), tst["timestamp"].max().strftime('%Y-%m-%d')
        print(f"fold: {fold} train: [{trn_start} - {trn_end}] val: [{val_start} - {val_end}]")



        model_dict, val = train_predict(all_data_in_fold, 
    ...
    # Retrain on all available data and run on test
    #all_trn = all_data.query(f'timestamp < "{test_start_date}"').copy()
    #X_trn_all, y_trn_all = create_io_features(all_trn, label='target')
    #trn_start, trn_end = all_trn["timestamp"].min().strftime('%Y-%m-%d'), all_trn["timestamp"].max().strftime('%Y-%m-%d')
    #print(f"inference: train: [{trn_start} - {trn_end}] test: [{tst_start} - {tst_end}]")
